...
>>>>>>> abaf5af68e837130a6d1d3977c4bf66854aba03d
...
export default function formReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case FILTER:
      ...
      let sportFilteredFields = [...state.allFieldsBackUp];
      if (newSportFilters.sport) {
        sportFilteredFields = sportFilteredFields.filter((f) =>
          f.sports.includes(newSportFilters.sport)
        );
      }
      if (newSportFilters.city) {
        sportFilteredFields = sportFilteredFields.filter(
          (f) => f.city === newSportFilters.city
        );
      }
      if (newSportFilters.horario) {
        sportFilteredFields = sportFilteredFields.filter((f) =>
          f.shift?.includes(newSportFilters.horario)
        );
      }
      return {
        ...state,
    ...
    case FILTER_PRICE_RANGE:
      const filteredFields = state.allFieldsBackUp.filter(
        (field) =>
          field.price >= action.payload.min && field.price <= action.payload.max
      );
      return {
        ...state,
        fieldData:
