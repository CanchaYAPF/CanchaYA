...
const { decodeGoogleTokenId, decodeJwtTokenId} = require("../../utils/decodedToken")//modularice el decifrado de tokens
...
const postField = async (name, image, sports, phone, address, city, paymentMethod, price, service, shift, token) => {
  try {
    ...
    if (existingField) {
      ...
      const tokenId=await decodeJwtTokenId(token)
      let userId= tokenId ? tokenId: await decodeGoogleTokenId(token);
